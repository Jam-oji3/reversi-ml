...
let input_color ic =
  let s = input_line ic in
    if s = "white" then Color.white
    else                Color.black

let output_color oc color =
  if color = Color.white then (output_string oc "white\n"; flush oc)
...
let string_of_hist (x : hist) = List.fold_left (fun r a -> string_of_opmove a ^ " " ^ r) "" x
let print_hist (x : hist) = print_endline (string_of_hist x)
...
and my_move (ic,oc) board color hist oname mytime =
  let pmove = play ~time_ms:mytime board color in
  let board = do_move board pmove color in
  let _ = output_command oc (Move pmove) in
  let _ = if !opt_verbose then
      (print_endline "--------------------------------------------------------------------------------";
       print_endline ("PMove: " ^ string_of_move pmove ^ " " ^ Color.string color);
...
and op_move (ic,oc) board color hist oname mytime =
  match input_command ic with
    | Move omove ->
      let board = do_move board omove (Color.opposite color) in
      let _ = if !opt_verbose then
          (print_endline "--------------------------------------------------------------------------------";
           print_endline ("OMove: " ^ string_of_move omove ^ " " ^ Color.string color);
           print_board board) in
...
and proc_end (ic,oc) board color hist oname wl n m r =
  ...
  let _ =
    printf "Your name: %s (%s)  Oppnent name: %s (%s).\n"
      (!opt_player_name ) (Color.string color)
      oname (Color.string (Color.opposite color)) in
