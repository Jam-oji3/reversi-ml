...
let pr fm =
  if !opt_log_level <> Silent then Format.printf fm
  else Format.ifprintf Format.std_formatter fm

(* Log level *)
let verbose f = if !opt_log_level = Verbose then f () else ()
...
let options =
  [
    ...
    ( "-t",
      Arg.Set_int opt_players_time,
      " time assigned to each player (in second; default = 600)" );
    ( "--timeout",
      Arg.Set_int opt_timeout,
      " time out, i.e. how long the server waits for a command (in \
       millisecond; default = 60000)" );
    ( "-T",
      Arg.Set_int opt_byoyomi,
      " byoyomi time (in millisecond; default = 500)" );
    ("-n", Arg.Set_int opt_players_number, " number of players (default = 2)");
    ( "-r",
      Arg.Set_int opt_rounds,
      " number of rounds (multiple of 2; default = 4)" );
    ( "-c",
      Arg.Set_int opt_concurrency,
      " number of maximum concurrent matches (default = 1)" );
  ]
...
let hidden =
  [
    ...
    ( "--byoyomi",
      ...
      " byoyomi time (in millisecond; default = 500)" );
    ( "--players",
      Arg.Set_int opt_players_number,
    ...
    ( "--concurrency",
      Arg.Set_int opt_concurrency,
      " number of maximum concurrent matches (default = 1)" );
    ("-s", Arg.Unit (fun () -> opt_log_level := Silent), "");
...
type client =
  | Client of
      Unix.file_descr * in_channel * out_channel * string ref * score ref

and score = { win : int; lose : int }
...
let send_client (Client (_, _, outchan, name, _)) =
  Printf.ksprintf (fun str ->
      verbose (fun () ->
          Format.eprintf "\x1b[36mSending to %s: %s\x1b[0m@." !name str);
...
let add_game_client (Client (_, _, _, _, score)) = function
  | "WIN" -> score := { win = !score.win + 1; lose = !score.lose }
  | "LOSE" -> score := { win = !score.win; lose = !score.lose + 1 }
  | _ -> ()
...
let put_waiting_msg = function
  ...
  | n ->
      verbose (fun () ->
          Format.eprintf "\x1b[34mWaiting %d connetion%s ...\x1b[0m@." n
            (if n > 1 then "s" else ""))

(* n人の総当り対戦カードを作る (ただし同じ組み合わせで逆のは作らない)*)
let make_cards n =
  let rec aux a = function 0 -> a | n -> aux ((n - 1) :: a) (n - 1) in
  List.concat
    (List.map
       (fun y ->
...
let string_of_error = function
  | PlayInvalidCommand _ -> "INVALID_COMMAND"
  | PlayInvalidMove _ -> "INVALID_MOVE"
  | PlayGiveUp _ -> "GIVEUP"
  | PlayTimeUp _ -> "TIEMUP"
...
and print_stat clients =
  pr "name,score,win,lose@.";
  (* statを表示 *)
  let longest =
    List.fold_left max 0
    @@ List.map (fun c -> String.length (name_of_client c)) clients
  in
  let s1, s2 =
    if !opt_log_level = Silent then ("", "") else ("\x1b[36m", "\x1b[0m")
  in
  List.iter
    (fun (Client (_, _, _, name, score)) ->
      Format.printf "%s%s, %d, %d, %d%s@." s1 (pad !name longest)
        (!score.win - !score.lose) !score.win !score.lose s2)
    clients

and session clients =
  (* XXX 既に消えてる人のことを考えてないけどまあいいや *)
  (* 各クライアントからopenを受け取る *)
  pr "%s Session started (%d players)@.@." (time_str ()) (List.length clients);
  try
    (* 名前を聞く *)
    List.iter
      (fun (Client (_, inchan, _outchan, nr, _)) ->
        ...
        | _ ->
            ...
            verbose (fun () ->
                Format.eprintf
                  "\x1b[33mReceived invalid command from %s\x1b[0m@." !nr);
            byebye clients;
            raise SessionEnd)
      clients;
    (* 総当り対戦 *)
    let cards = make_cards !opt_players_number in
    let cards2 = List.map (fun (x, y) -> (y, x)) cards in
    play_all 0 cards cards2 clients;
    print_stat clients;
    byebye clients;
    raise SessionEnd
  with SessionEnd ->
    pr "%s Session ended@." (time_str ());
    List.iter close_client clients
      ...
      | _ -> (
          ...
          let rec search_card acc = function
            ...
            | (b, w) :: cards' ->
                ...
                else search_card ((b, w) :: acc) cards'
          in
...
and play round round_max gamenum game_max bc wc pipewrite =
  pr "%s Round %d/%d, Game %d/%d (%s vs %s)@." (time_str ()) (round + 1)
    round_max gamenum game_max (name_of_client bc) (name_of_client wc);
  (* Boarsを準備 *)
  ...
  let wait_for_move (Client (_, inchan, _, _, _) as cl) time =
    ...
    | _ ->
        (* invalid command *)
        Printf.eprintf "INVALID COMMAND %s\n" (string_of_command_type com);
        raise (PlayInvalidCommand cl)
  in
  (* 前回パスしたかどうかのフラグ *)
  let pass_flg = ref false in
  (* プレイを行う *)
  let rec plays ((cl_curr, time_curr, color_curr) as curr)
      ((cl_oppo, _time_oppo, _color_oppo) as oppo) =
    ...
    | Mv (x, y) ->
        ...
        if is_valid_move !board color_curr (x, y) then (
          (* 許す *)
          board := do_move !board m color_curr;
          send_client cl_curr "ACK %d\n" !time_curr;
          (* 時間が切れていないか? *)
          if !time_curr < -1 * !opt_byoyomi then raise (PlayTimeUp cl_curr);
          (* 秒読みモード *)
          if !time_curr <= 0 then time_curr := !opt_byoyomi;
    ...
    | Pass ->
        if valid_moves !board color_curr = [] then
          ...
          (* 許さない!!!!!!!!!!!111111!1!!!111!11111 *)
          raise (PlayInvalidMove cl_curr)
    | GiveUp -> raise (PlayGiveUp cl_curr)
  and normal_end_game () =
    (* 終了処理 *)
    let black_count = count !board Color.black in
    let white_count = count !board Color.white in
    let black_result =
      if black_count > white_count then "WIN"
      else if black_count < white_count then "LOSE"
      else "TIE"
    in
    let white_result =
      if black_count < white_count then "WIN"
  ...
  and end_game black_result white_result reason =
    ...
    (* 結果を教える *)
    send_client bc "END %s %d %d %s\n" black_result black_count white_count
      reason;
    send_client wc "END %s %d %d %s\n" white_result white_count black_count
      reason;
    pr
      "%s \x1b[32mRound %d/%d, Game %d/%d Result: %s %s (%d), %s %s (%d); \
       (%d-%d, %s)\x1b[0m@."
      (time_str ()) (round + 1) round_max gamenum game_max (name_of_client bc)
      black_result !time_bc (name_of_client wc) white_result !time_wc
      black_count white_count reason
  in
  try plays (bc, time_bc, Color.black) (wc, time_wc, Color.white)
  with
  | (PlayInvalidCommand cl | PlayInvalidMove cl | PlayGiveUp cl | PlayTimeUp cl)
    as e
  ->
    let black_result =
      if id_of_client cl = id_of_client bc then "LOSE" else "WIN"
    in
    let white_result =
      if id_of_client cl = id_of_client wc then "LOSE" else "WIN"
    in
    end_game black_result white_result (string_of_error e)
...
and byebye clients =
  ...
  List.iter
    (fun (Client (_, _, _, name, score)) ->
      Buffer.add_string stat
        (Printf.sprintf " %s %d %d %d" !name (!score.win - !score.lose)
           !score.win !score.lose))
    clients;
  let stat = Buffer.contents stat in
  List.iter (fun c -> send_client c "BYE%s\n" stat) clients
  ...
  let rec w clients n =
    if n <= 0 then (
      ...
      Unix.setsockopt_float sock Unix.SO_RCVTIMEO
        (float_of_int !opt_timeout /. 1000.0);
      let inchan = Unix.in_channel_of_descr sock in
      let outchan = Unix.out_channel_of_descr sock in
      let kari_client_name =
        Printf.sprintf "Client #%d" (!opt_players_number - n + 1)
      in
      let c =
        Client
          ( sock,
